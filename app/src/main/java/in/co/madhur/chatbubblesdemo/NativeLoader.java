...
public class NativeLoader {
    ...
    private static boolean loadFromZip(Context context, File destDir, File destLocalFile, String folder) {
        try {
            ...
            Log.e(in.co.madhur.chatbubblesdemo.Constants.TAG, e.getMessage());
        }
    ...
    public static synchronized void initNativeLibs(Context context) {
        if (nativeLoaded) {
            Log.d("NativeLoad ","Native libs already loaded");
            return;
        }
        ...
        try {
            ...
            try {
                if (Build.CPU_ABI.equalsIgnoreCase("armeabi-v7a")) {
                    ...
                    folder = "armeabi";
                    Log.e(in.co.madhur.chatbubblesdemo.Constants.TAG,"Folder error1");
                    //FileLog.e("tmessages", "Unsupported arch: " + Build.CPU_ABI);
                }
            } catch (Exception e) {
                //  FileLog.e("tmessages", e);
                Log.e(in.co.madhur.chatbubblesdemo.Constants.TAG,"Folder error2 "+e.getMessage());
                folder = "armeabi";
            }
            ...
            if (destLocalFile != null && destLocalFile.exists()) {
                try {
                    ...
                    Log.e(in.co.madhur.chatbubblesdemo.Constants.TAG,"Folder error3 "+e.getMessage());
                    Log.e(in.co.madhur.chatbubblesdemo.Constants.TAG, e.getMessage());
                }
            ...
            if (loadFromZip(context, destDir, destLocalFile, folder)) {
                Log.e(in.co.madhur.chatbubblesdemo.Constants.TAG,"Folder error3.5 already loaded");
                return;
            }
            ...
            Log.e(in.co.madhur.chatbubblesdemo.Constants.TAG,"Folder error4 "+e.getMessage());
            e.printStackTrace();
        }

        try {
            Log.d("LOADING 2",LIB_NAME);
            System.loadLibrary(LIB_NAME);
            nativeLoaded = true;
        } catch (Error e) {
            Log.e(in.co.madhur.chatbubblesdemo.Constants.TAG,"Folder error5 "+e.getMessage());
