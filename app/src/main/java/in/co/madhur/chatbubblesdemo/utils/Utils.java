...
public class Utils {
    ...
    public static ArrayList<ContactModel> getPhoneContactDetails(Context context, String filter, int offset, int limit, Uri uri,String msisdn) {
            ...
            if (cursor.moveToFirst()) {
                do {
                 /*   if(count>10)
                    ...
                    if(phone!=null && !phone.equalsIgnoreCase(msisdn)) {
    ...
    public static ArrayList<ContactModel>   search(Context context, String query,String msisdn){
    ...
    public static Bitmap decodeBase64old(String input)
    {
        try {
            byte[] decodedByte = Base64.decode(input, Base64.DEFAULT);;//Base64.decode(input, 0);
            return getImage(decodedByte);//BitmapFactory.decodeByteArray(decodedByte, 0, decodedByte.length);
        }
        catch(Exception e) {
    e.printStackTrace();
        }
