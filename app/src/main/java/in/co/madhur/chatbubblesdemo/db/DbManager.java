...
public class DbManager extends SQLiteOpenHelper {
    ...
    public  ArrayList<ChatMessage> getOfflineChats(){
        ArrayList<ChatMessage> listMessages=new ArrayList<ChatMessage>();
        try {

            SQLiteDatabase sqldb = getWritableDatabase();
            String query = " SELECT *  FROM " + DB.Chat.TABLE + "  where " + DB.Chat.STATUS + " "
                    + " = ? order by " + DB.Chat._ID + " ASC";
            Log.d("GETTING CHATS: ",query);
            Cursor cursor = sqldb.rawQuery(query, new String[]{Status.NOT_SENT.toString()});
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                do {
                    ...
                    chatMessage.setMessageID(cursor.getString(cursor.getColumnIndex(DB.Chat.messageID)));
    ...
    public  ArrayList<ChatMessage> getRecentChats(){
        ArrayList<ChatMessage> listMessages=new ArrayList<ChatMessage>();
        try {

            SQLiteDatabase sqldb = getWritableDatabase();
            String query = " SELECT "+DB.Chat.TO+"," +
                    " (SELECT "+DB.Chat.MESSAGE+" FROM "+DB.Chat.TABLE+" where "+DB.Chat.TO+"= chat."+DB.Chat.TO+" ORDER BY "+DB.Chat._ID+" DESC LIMIT 1) AS "+DB.Chat.MESSAGE+",  " +
                    " (SELECT "+DB.Chat.STATUS+" FROM "+DB.Chat.TABLE+" where "+DB.Chat.TO+"= chat."+DB.Chat.TO+" ORDER BY "+DB.Chat._ID+" DESC LIMIT 1) AS "+DB.Chat.STATUS+", " +
                    " (SELECT "+DB.Chat.DATE_CREATED+" FROM "+DB.Chat.TABLE+" where "+DB.Chat.FROM+"= chat."+DB.Chat.FROM+" ORDER BY "+DB.Chat._ID+" DESC LIMIT 1) AS "+DB.Chat.DATE_CREATED+" " +
                    "FROM " + DB.Chat.TABLE + " chat GROUP BY "+DB.Chat.TO+" order by "+DB.Chat.DATE_CREATED+" DESC";
            Log.d("GETTING CHATS PER : ",query);
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                do {
                    ...
                    String Name=cursor.getString(cursor.getColumnIndex(DB.Chat.TO));
                    try {
                        ArrayList<ContactModel> contactModel = MessageFragment.getContacts(0, 20, Name);
                        Log.d("CHAT USER",contactModel.get(0).getName());
                         Name = contactModel.get(0).getName();
                    }
                    catch(Exception e)
                    {
                    ...
                    ChatMessage chatMessage = new ChatMessage();
                    chatMessage.setMessage(cursor.getString(cursor.getColumnIndex(DB.Chat.MESSAGE)));
                    chatMessage.setTo(cursor.getString(cursor.getColumnIndex(DB.Chat.TO)));
                    chatMessage.setFrom(Name);
                    chatMessage.setMessageStatus(Status.valueOf(cursor.getString(cursor.getColumnIndex(DB.Chat.STATUS))));
                    chatMessage.setMessageTime(new Date(cursor.getString(cursor.getColumnIndex(DB.Chat.DATE_CREATED))));
                    listMessages.add(chatMessage);
